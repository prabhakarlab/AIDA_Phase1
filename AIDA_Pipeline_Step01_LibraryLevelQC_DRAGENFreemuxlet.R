### Standard pipeline for processing of AIDA DRAGEN + Freemuxlet datasets
### This code is adapted from the standard pipeline for processing DRAGEN
### datasets to account for libraries where *one* genetic demultiplexing
### genotype is missing. We identify Freemuxlet genotype clusters 
### and use this to reannotate the donor identity of cells - corresponding
### to the donor with the missing genotype.
...
### Ensure that input files are re-named: barcodes.tsv, genes.tsv, matrix.mtx

##### USER INPUT REQUIRED: FIELD 1 library / libraries of interest #####
### Enter a vector of library names to loop through

vec_str_name <- c("SG_HEL_B020_L001", "SG_HEL_B020_L002")
...
for (str_name in vec_str_name){
  ...
  df_demuxlet <- read.table(paste0(str_link_dataset, str_name, "_5GEX.scRNA.barcodeSummary.tsv"),
                            sep = "\t", header = TRUE)
  df_freemuxlet <- read.table(paste0(str_link_dataset, str_name, ".clust1.samples"),
                              sep = "\t", header = TRUE)
  ...
  rca_PBMC <- createRCAObjectFrom10X(str_link_dataset)
  ...
  write.table(data.frame(barcode = colnames(rca_PBMC$raw.data)), 
              file = paste0(str_name, "_Barcodes_NODG_300.txt"), 
  ...
  ##### Identifying genetic doublets from DRAGEN genetic demultiplexing #####
  
  df_demuxlet <- df_demuxlet[which(df_demuxlet$Barcode %in% colnames(rca_PBMC$raw.data)), ]
  df_demuxlet$Type <- unlist(lapply(df_demuxlet$SampleIdentity,
                                    function(x) unlist(strsplit(x, split = ":"))[1] ))
  
  ##### Matching Freemuxlet output with DRAGEN genetic demultiplexing output #####
  
  df_freemuxlet <- df_freemuxlet[which(df_freemuxlet$BARCODE %in% colnames(rca_PBMC$raw.data)), ]
  
  df_demuxlet_subset <- df_demuxlet[, c("Barcode", "Type", "SampleIdentity")]
  df_freemuxlet_subset <- df_freemuxlet[, c("BARCODE", "DROPLET.TYPE", "SNG.BEST.GUESS")]
  colnames(df_freemuxlet_subset) <- c("Barcode", "Freemuxlet_Type", "Sample")
  df_demuxlet_subset[which(df_demuxlet_subset$Type == "AMB"), "SampleIdentity"] <- "AMB"
  df_demuxlet_subset[which(df_demuxlet_subset$Type == "MIX"), "SampleIdentity"] <- "MIX"
  df_freemuxlet_subset[which(df_freemuxlet_subset$Freemuxlet_Type == "AMB"), "Sample"] <- "AMB"
  df_freemuxlet_subset[which(df_freemuxlet_subset$Freemuxlet_Type == "DBL"), "Sample"] <- "DBL"
  df_demultiplex_merged <- merge(df_demuxlet_subset, df_freemuxlet_subset, by = "Barcode")
  
  # Wrangle DRAGEN output and Freemuxlet output to obtain alluvial plot
  
  df_demuxlet_by_freq <- data.frame(table(df_demuxlet_subset$SampleIdentity))
  df_demuxlet_by_freq[order(df_demuxlet_by_freq$Freq), ]
  df_freemuxlet_by_freq <- data.frame(table(df_freemuxlet_subset$Sample))
  df_freemuxlet_by_freq[order(df_freemuxlet_by_freq$Freq), ]
  
  # Order DRAGEN output and Freemuxlet output by number of barcodes per genotype for neater alluvial plot
  
  df_demultiplex_merged$SampleIdentity <- factor(df_demultiplex_merged$SampleIdentity, 
                                                 levels = df_demuxlet_by_freq[order(df_demuxlet_by_freq$Freq), ]$Var1)
  df_demultiplex_merged$Sample <- factor(df_demultiplex_merged$Sample, 
                                         levels = df_freemuxlet_by_freq[order(df_freemuxlet_by_freq$Freq), ]$Var1)
  
  df_alluvial <- as.data.frame(table(df_demultiplex_merged$SampleIdentity, df_demultiplex_merged$Sample))
  colnames(df_alluvial) <- c("DRAGEN", "Freemuxlet", "Freq")
  write.table(df_alluvial, paste0(str_name, "_DataFrame_Alluvial_DRAGEN_Freemuxlet_concordance.txt"),
              col.names = TRUE, row.names = FALSE, sep = "\t", quote = FALSE)
  
  ggplot(data = df_alluvial, 
         aes(axis1 = DRAGEN, axis2 = Freemuxlet, y = Freq)) +
    geom_alluvium(aes(fill = DRAGEN), width = 1/12) +
    geom_stratum(width = 0.1) +
    geom_text(stat = "stratum",
              aes(label = after_stat(stratum))) +
    scale_x_discrete(limits = c("DRAGEN", "Freemuxlet"),
                     expand = c(0.05, 0.05)) +
    ggtitle(paste0(str_name, " Alluvial plot of DRAGEN and Freemuxlet genotype concordance"))
  ggsave(paste0(str_name, "_DRAGEN_Freemuxlet_Concordance.pdf"), width = 25, height = 12.5)
  
  df_alluvial <- df_alluvial %>% group_by(Freemuxlet) %>% top_n(n = 2, wt = Freq)
  df_alluvial <- df_alluvial[order(df_alluvial$Freemuxlet), ]
  
  ##### Identifying majority vote of DRAGEN assignment for each Freemuxlet genotype cluster #####
  
  df_alluvial <- (df_alluvial %>% filter(Freq == max(Freq)))
  
  for (freemuxlet_cluster in 0:15){
    if (df_alluvial[which(df_alluvial$Freemuxlet == freemuxlet_cluster), "DRAGEN"] == "MIX") {
      str_missing_genotype <- freemuxlet_cluster
    }
  }
  
  ##### Re-assigning droplet type and sample identity in DRAGEN data frame #####
  ### df_demuxlet$Barcode %in% vec_barcodes_reassign
  ### All barcodes identified by Freemuxlet as being singlets 
  ### that correspond to the missing demultiplexing genotype
  ### are re-assigned as singlets (SNG) and SNG:<Freemuxlet cluster>
  
  vec_barcodes_reassign <- df_freemuxlet$BARCODE[which((df_freemuxlet$DROPLET.TYPE == "SNG") &
                                                         (df_freemuxlet$SNG.BEST.GUESS == str_missing_genotype))]
  df_demuxlet$Type[df_demuxlet$Barcode %in% vec_barcodes_reassign] <- "SNG"
  df_demuxlet$SampleIdentity[df_demuxlet$Barcode %in% vec_barcodes_reassign] <- paste0("SNG:Freemuxlet_", str_name, "_CLUST_", str_missing_genotype)
  
  df_type <- as.data.frame(table(df_demuxlet$Type))
  
  write.table(df_type, paste0(str_name, "_DRAGEN_summary_AMB_MIX_SNG_Freemuxlet_adapted.txt"),
              col.names = FALSE, row.names = FALSE, sep = "\t", quote = FALSE)
  
  vec_demuxlet_singlet <- df_demuxlet[which(df_demuxlet$Type=="SNG"), "Barcode"]
  
  # Estimate doublet rate: MIX + AMB droplets
  
  number_estimated_doublet_rate <- ((sum(df_demuxlet$Type == "MIX") + 
                                       sum(df_demuxlet$Type == "AMB"))/length(df_demuxlet$Type))*int_number_of_samples/(int_number_of_samples-1)
  ...
  data_PBMC <- Read10X(data.dir = str_link_dataset)
  ...
  # Add ground truth (GT) from DRAGEN genetic demultiplexing to Seurat object
  ...
  # DRAGEN genetic demultiplexing and DoubletFinder
  ...
  pdf(paste0(str_name, "_Library_QC_Plot10_UMAP_Seurat_Singlets_Doublets_DRAGEN_DoubletFinder.pdf"))
  print(ggplot() + 
...
\nGenetic Doublets and Doublet Finder Doublets (blue)"))
  ...
  # Plot QC metrics for DRAGEN and DoubletFinder singlets and doublets
  ...
  # DRAGEN genetic demultiplexing doublet QC metrics
  ...
  ggplot(df_all_NODG_UMI_pMito_DRAGEN_Doublet, aes(x = NODG, y = pMito)) + 
    ...
    geom_density_2d() + theme_bw(11) + xlim(0, 6000) + ylim(0, 0.1) +
    ggtitle(paste0(str_name, " pMito and NODG \nfor DRAGEN doublets"))
  ggsave(paste0(str_name, "_Library_QC_Plot12_pMito_NODG_DRAGEN_Doublets.png"), width = 6, height = 5)
  ...
  # Plot number of DRAGEN+DoubletFinder singlets per donor
  
  df_demuxlet_by_donor <- df_demuxlet[which(df_demuxlet$Type == "SNG"), c("Barcode", "SampleIdentity")]
  ...
  ggplot(df_singlets_per_donor, aes(x = Donor, y = Freq)) +
    ...
    ggtitle(paste("Bar chart of Demuxlet + Freemuxlet donor assignments for \n(Demuxlet and DoubletFinder) singlets in\n", str_name)) + 
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
  ggsave(paste0(str_name, "_Library_QC_Plot14_Singlets_by_donors_with_Freemuxlet_cluster.png"), units = "in", width = 16, height = 6.66)
  ...
  ### Save data frame with singlet/doublet (DRAGEN, DoubletFinder) information: 
  ### NODG and pMito; cell numbers > NODG 300 (nrow indicative of capture rate); 
  ### Doublet rates (DRAGEN/Genetic, DoubletFinder, either)
  ...
  ### seurat_PBMC_subset[[]]: Singlet/doublet (DRAGEN, DoubletFinder) information: 
  ### NODG and pMito; cell numbers > NODG 300 (nrow indicative of capture rate); 
  ### Doublet rates (DRAGEN/Genetic, DoubletFinder, either); RCAv2 information
  
  rownames(df_demuxlet) <- df_demuxlet$Barcode
  df_demuxlet_singlet <- df_demuxlet[vec_final_singlet_barcode, ]
  df_demuxlet_singlet$Genotyping_ID <- unlist(lapply(df_demuxlet_singlet$SampleIdentity,
                                                     function(x) unlist(strsplit(x, split = "SNG:"))[2] ))
  ...
  df_singlets_per_donor$Donor <- as.character(df_singlets_per_donor$Donor)
  df_singlets_per_donor$Genotyping_ID <- unlist(lapply(df_singlets_per_donor$Donor,
                                                       function(x) unlist(strsplit(x, split = "SNG:"))[2]))
  ...
  # Save Seurat and RCA objects with all DRAGEN and DoubletFinder singlets
