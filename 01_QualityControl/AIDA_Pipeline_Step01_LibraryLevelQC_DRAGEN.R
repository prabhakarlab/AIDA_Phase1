### Standard pipeline for processing of AIDA DRAGEN datasets
...
### Either use an input file containing library names of interest, or
### assign specific library names to vec_str_name

file_libraries <- read.table("/mnt/sdd/AIDA_Pipeline_v2/files_global/AIDA_Phase1_CodePipeline_Assignment.txt", 
                             header = TRUE, sep = "\t")
file_libraries <- file_libraries[which(file_libraries$LibraryQC_Code == "DRAGEN"), ]
str_code_part <- "Part_1" ### or "Part_2" or "Part_3"

vec_str_name <- file_libraries$Library[which(file_libraries$CellTypeQC_Part == str_code_part)]
...
for (str_name in vec_str_name){
  ...
  write.table(df_type, paste0(str_name, "_DRAGEN_summary_AMB_MIX_SNG.txt"),
