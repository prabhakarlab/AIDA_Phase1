### AIDA Phase 1 Data Freeze v2 Differential gene expression analysis: version 7 January 2024
...
df_AIDA_metadata <- df_AIDA_metadata[!(df_AIDA_metadata$Ancestry == "European"), ]
...
for (str_subtype in c("CD16+_NK", "CD4+_T_cm", "CD4+_T_cyt", "CD4+_T_em", "CD4+_T_naive", 
  ...
  df_design <- df_design[, c("DCP_ID", "Age", "Sex", "Ancestry", "Batch")]
  ...
  df_design$SG_Chinese[which(df_design$Ancestry == "SG_Chinese")] <- "SG_Chinese"
  df_design$Not_SG_Chinese <- 1
  df_design$Not_SG_Chinese[which(df_design$Ancestry == "SG_Chinese")] <- 0
  
  df_design$SG_Malay <- "Not_SG_Malay"
  df_design$SG_Malay[which(df_design$Ancestry == "SG_Malay")] <- "SG_Malay"
  df_design$Not_SG_Malay <- 1
  df_design$Not_SG_Malay[which(df_design$Ancestry == "SG_Malay")] <- 0
  
  df_design$SG_Indian <- "Not_SG_Indian"
  df_design$SG_Indian[which(df_design$Ancestry == "SG_Indian")] <- "SG_Indian"
  df_design$Not_SG_Indian <- 1
  df_design$Not_SG_Indian[which(df_design$Ancestry == "SG_Indian")] <- 0
  ...
  for (str_Ancestry in c("SG_Chinese", "SG_Malay", "SG_Indian")){
    df_design[, str_Ancestry] <- factor(df_design[, str_Ancestry], 
                                         levels=c(paste0("Not_", str_Ancestry), 
                                                  str_Ancestry))
    design <- model.matrix(formula(paste0("~Sex + Age + Batch + ", str_Ancestry)), 
                           data=df_design)
    ...
    write.table(lrt$table, 
                file = paste("edgeR", str_subtype, str_Ancestry, ".txt", sep = "_"), 
    ...
    png(paste("Rplot_MDS_Sex", str_Ancestry, str_subtype, ".png", sep = "_"))
    plotMDS(y, col=colours[group], pch=points[group])
    title("MDS plot of sex")
    dev.off()
    
    group <- factor(y$design[, paste0(str_Ancestry, str_Ancestry)])
    colours <- c("black", "pink")
    points <- c(15, 16)
    png(paste("Rplot_MDS_Ancestry", str_Ancestry, str_subtype, ".png", sep = "_"))
    plotMDS(y, col=colours[group], pch=points[group])
    title("MDS plot of ancestry")
    dev.off()
    
    # MA plot
    png(paste("Rplot_MA_Ancestry", str_Ancestry, str_subtype, ".png", sep = "_"))
...
for (str_filename in vec_filenames){
  ...
  if (dim(df_edgeR)[1] > 0){
    ...
    df_edgeR$Ancestry <- strsplit(str_to_split, split = "_")[[1]][length(strsplit(str_to_split, split = "_")[[1]])]
...
for (str_ancestry in c("SG_Chinese", "SG_Indian", "SG_Malay")){
  ...
  for (str_celltype in c("atypical_B", "memory_B_IGHMhi", "memory_B_IGHMlo", "naive_B", 
    ...
    df_test <- read.table(paste("edgeR", 
                                str_celltype, str_ancestry, ".txt", sep = "_"), header = TRUE, sep = "\t")
    ...
    ggplot(data = df_test, 
      ...
      ggtitle(paste0("Differentially expressed genes in ", str_celltype,  
                     ": \n", str_ancestry, " versus other Singapore ancestries"))
    ggsave(paste("Volcano_GSEA_output/Rplot_Fig5_VolcanoPlot", 
                 str_ancestry, str_celltype, ".pdf", sep = "_"), 
    ...
    if (dim(gse@result)[1] > 0){
      dotplot(gse, showCategory = 10, split = ".sign") + facet_grid( . ~ .sign) + 
        ggtitle(paste0("GSEA (GO biological process) of gene expression in ", str_celltype,  
                       ": \n", str_ancestry, " versus other Singapore ancestries"))
      ggsave(paste("Volcano_GSEA_output/Rplot_Fig5_GSEA_log10pvalue", str_ancestry, str_celltype, ".pdf", sep = "_"), 
...
str_ancestry <- "SG_Indian"
str_celltype <- "MAIT"

str_ancestry <- "SG_Chinese"
str_celltype <- "CD16+_NK"

str_ancestry <- "SG_Malay"
str_celltype <- "CD14+_Monocyte"

dotplot(gse, showCategory = 5, split = ".sign") + facet_grid( . ~ .sign) + 
  ggtitle(paste0("GSEA (GO biological process) of gene expression in ", str_celltype,  
                 ": \n", str_ancestry, " versus other Singapore ancestries"))
ggsave(paste("Rplot_Fig5_GSEA_log10pvalue", str_ancestry, str_celltype, ".pdf", sep = "_"), 
       height = 6.36, width = 11.96, units = "in")
...
### Getting UTS2 values from each Ancestry
...
for (str_ancestry in c("SG_Chinese", "SG_Indian", "SG_Malay")){
  for (str_celltype in c("atypical_B", "memory_B_IGHMhi", "memory_B_IGHMlo", "naive_B", 
    ...
    if (dim(df_test[which(rownames(df_test) == "UTS2"), ])[1] > 0){
      if ((str_ancestry == "SG_Chinese") & (str_celltype == "memory_B_IGHMlo")){
        print("Yes")
        df_UTS2 <- df_test[which(rownames(df_test) == "UTS2"), ]
        vec_UTS2_ancestry <- c(vec_UTS2_ancestry, str_ancestry)
...
df_UTS2$Ancestry <- vec_UTS2_ancestry
...
ggplot(df_UTS2, aes(y = logFC, x = logCPM, colour = Ancestry, shape = Subtype)) + 
  geom_point(size = 4.9) + 
...
### Getting TIFA values from each Ancestry
...
for (str_ancestry in c("SG_Chinese", "SG_Indian", "SG_Malay")){
  for (str_celltype in c("atypical_B", "memory_B_IGHMhi", "memory_B_IGHMlo", "naive_B", 
    ...
    if (dim(df_test[which(rownames(df_test) == "TIFA"), ])[1] > 0){
      if ((str_ancestry == "SG_Chinese") & (str_celltype == "atypical_B")){
        print("Yes")
        df_TIFA <- df_test[which(rownames(df_test) == "TIFA"), ]
        vec_TIFA_ancestry <- c(vec_TIFA_ancestry, str_ancestry)
...
df_TIFA$Ancestry <- vec_TIFA_ancestry
...
ggplot(df_TIFA[which(df_TIFA$Subtype %in% c("naive_B", "memory_B_IGHMhi",
                                            "memory_B_IGHMlo", "atypical_B")), ], 
       aes(y = logFC, x = logCPM, colour = Ancestry, shape = Subtype)) + 
  geom_point(size = 3) + 
