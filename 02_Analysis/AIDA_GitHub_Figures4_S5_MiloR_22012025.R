### AIDA Phase 1 Data Freeze v2 Cell neighbourhood enrichment MiloR analysis: version 22 January 2025
...
##### Consider only donors with at least 800 cells and country-ethnicity combinations with at least 50 donors per combination #####
...
df_all_metadata <- df_all_metadata[!(df_all_metadata$ethnicity == "European"), ]

##### Reading in cell subtype data, and including metadata and annotations (human diversity, cell type annotation) #####

for (str_ethnicity in unique(df_all_metadata$ethnicity)){
  df_all_metadata[ , str_ethnicity] <- 0
  df_all_metadata[ , str_ethnicity][which(df_all_metadata$ethnicity == str_ethnicity)] <- 1
}
...
for (str_ethnicity in unique(seurat_AIDA_subset$ethnicity)){
  seurat_AIDA_subset[[str_ethnicity]] <- 0
  seurat_AIDA_subset[[str_ethnicity]] <- df_barcodes_to_keep[colnames(seurat_AIDA_subset), str_ethnicity]
}
...
##### Design matrix for covariates of interest: sex, age, and ethnicity, option of having batch covariate #####
traj_design <- 
  data.frame(colData(milo_AIDA_subset))[, c("DCP_ID", "Batch", "Sex",
                                            "Age", "Ages_19_to_32", "Ages_50_to_77",
                                            "ethnicity", unique(df_barcodes_to_keep$ethnicity))]
traj_design <- distinct(traj_design)
rownames(traj_design) <- traj_design$DCP_ID

### Note that introducing batch as covariate alongside ethnicity would result in collinearity in covariates
...
### to avoid memory + computational time issues
  ...
  da_results <- testNhoods(milo_AIDA_subset, 
                           design = ~ Age + ethnicity + Female, 
                           design.df = traj_design, fdr.weighting = "graph-overlap")
  str_covariate_comparison <- "Female"
  function_beeswarm()
  function_plot_log2_meanfoldchange()
  
  ##### Testing for impact of ethnicity on cell neighbourhood abundance: SG_Chinese, SG_Malay, SG_Indian, Japanese, Korean, Thai #####
  ### For MiloR v1.5, used fdr.weighting = "graph-overlap" in testNhoods function 
  ### to avoid memory + computational time issues
  ...
  da_results <- testNhoods(milo_AIDA_subset, 
                           design = ~ ethnicity + Sex + Ages_19_to_32, 
                           design.df = traj_design, fdr.weighting = "graph-overlap")
  function_plot_log2_meanfoldchange()
  ...
  da_results <- testNhoods(milo_AIDA_subset, 
                           design = ~ ethnicity + Sex + Ages_50_to_77, 
                           design.df = traj_design, fdr.weighting = "graph-overlap")
  function_beeswarm()
  function_plot_log2_meanfoldchange()
...
##### For analysis of AIDA Singapore donor samples subsetted from Seurat objects #####

str_k <- 375  ### Try k = 300 with 200 Singapore donors - peak at around 700; try k = 350 - peak around 900
vec_majorcelltypes <- c("B", "Myeloid_pDC", "TNK_CD4", "TNK_nonCD4")

##### Consider only donors with at least 800 cells and country-ethnicity combinations with at least 40 donors per combination #####
##### INPUT REQUIRED: READ IN AIDA METADATA AS df_all_metadata #####
...
### Analysis of AIDA Singapore donor cells

df_all_metadata <- df_all_metadata[(df_all_metadata$Country == "SG"), ]
...
for (str_celltype in c("Myeloid_pDC", "TNK_CD4", "TNK_nonCD4")){
  ...
  ##### Running MiloR to assess influence of covariates on cell state abundance ######
  ...
  function_beeswarm <- function(){
    ...
    plotDAbeeswarm(da_results, group.by = "Annotation_Level3") +
      ggtitle(paste0("Cell state enrichment of ", str_celltype, " in ", str_covariate_comparison))
    ggsave(paste("Rplot_MiloR_Beeswarmby_Annotation_Level3", str_covariate_comparison, 
                 str_celltype, "k", str_k, ".png", sep = "_"), 
           width = 26.68, height = 11.24, units = "in")
  ...
  function_plot_log2_meanfoldchange <- function(){
    ...
    ### Capping plotted log2(mean fold change) at |1|
    df_UMAP[(df_UMAP$log2_MeanFoldChange > 1),"log2_MeanFoldChange"] <- 1
    df_UMAP[(df_UMAP$log2_MeanFoldChange < -1),"log2_MeanFoldChange"] <- -1
    ...
    ggplot()+
      geom_point(df_UMAP, 
                 mapping = aes(x = UMAP_1, y = UMAP_2, 
                               colour = log2_MeanFoldChange), 
                 size = 0.5, stroke = 0.1) +
      scale_colour_gradientn(colours = c("#00A600", "#E6E600", "#F2F2F2", "#EEB99F", "#EAB64E"),
                             values = c(1, 0.54, 0.5, 0.46, 0),  
                             limits = c(-1, 1)) +
      theme(panel.grid.major = element_blank(), 
            panel.grid.minor = element_blank(),
            ...
            axis.line = element_line(colour = "black")) + 
      ggtitle(paste0("Enrichment of ", str_celltype, " cells for ", str_covariate_comparison))
    ggsave(paste("Testcolourscale_RPlot_Figure_MiloR_UMAP_log2_MeanFoldChange", 
                 str_celltype, "k", str_k, str_covariate_comparison, 
                 ".png", sep = "_"), device = "png")
  }
  
  str_covariate_comparison <- "Chinese"
  
  da_results <- testNhoods(milo_AIDA_subset, 
                           design = ~ Age + Sex + Chinese, 
                           design.df = traj_design, fdr.weighting = "graph-overlap")
  function_beeswarm()
  function_plot_log2_meanfoldchange()
  
  str_covariate_comparison <- "Malay"
  
  da_results <- testNhoods(milo_AIDA_subset, 
                           design = ~ Age + Sex + Malay, 
                           design.df = traj_design, fdr.weighting = "graph-overlap")
  function_beeswarm()
  function_plot_log2_meanfoldchange()
  write.table(da_results, paste("df_daresults", 
                                str_celltype, "k", str_k, str_covariate_comparison, 
                                ".txt", sep = "_"), quote = FALSE, sep = "\t", col.names = TRUE, row.names = FALSE)
  
  str_covariate_comparison <- "Indian"
  
  da_results <- testNhoods(milo_AIDA_subset, 
                           design = ~ Age + Sex + Indian, 
                           design.df = traj_design, fdr.weighting = "graph-overlap")
  function_beeswarm()
  function_plot_log2_meanfoldchange()
  write.table(da_results, paste("df_daresults", 
