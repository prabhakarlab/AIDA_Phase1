### AIDA Phase 1 Data Freeze v2 Figure 1 Cell state enrichment analysis: version 7 January 2024
...
for (str_name in vec_library_names[1:length(vec_library_names)]){
  ...
  seurat_library_subset$Ancestry <- df_consented_genotypes[seurat_library_subset$Genotyping_ID, "Ancestry"]
...
### Comparisons of ancestries

vec_Asian_Ancestry <- c("Japanese", "Korean", "SG_Chinese", "SG_Malay", "SG_Indian", "Thai")

for (ref in vec_Asian_Ancestry){
  
  ref_num <- length(which(seurat_AIDA_subset$Ancestry==ref))
  
  # calculate number of reference cells in each cell's 500 neighbours
  graph_nnMat_final.ref <- graph_nnMat_final[, names(which(seurat_AIDA_subset$Ancestry==ref))]
  ...
  graph_nnMat_final.comp <- graph_nnMat_final[, names(which(seurat_AIDA_subset$Ancestry!=ref))]
  ...
  comp_FC <- length(which(seurat_AIDA_subset$Ancestry!=ref)) / ref_num
